...
export class UserShopsComponent {
  ...
  ngOnInit() {
    this.userid &&
      this.http.get(`users/${this.userid}/shops`).subscribe((resp) => (this.usershops = resp.json() as Shop[]));
  ...
  saveForm() {
    let shop1 = this.selectedItem.shopid;
    for (let i = 0; i < shop1.length; i++) {
      let shop = {
        shopid: shop1[i],
      };
      console.log(this.selectedItem.shopid);
